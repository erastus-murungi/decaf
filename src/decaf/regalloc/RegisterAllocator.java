...
public class RegisterAllocator {
  ...
  public RegisterAllocator(ProgramIr programIr) {
    ...
    var linearScan = new LinearScan(
        List.copyOf(X86Register.regsToAllocate),
        getLiveIntervalsManager().methodToLiveIntervalsMap
    );
    linearScan.allocate();
  ...
  private static Map<Instruction, Set<X86Register>> computeInstructionToLiveRegistersMap(
      ProgramIr programIr,
      List<LiveInterval> liveIntervals,
      Map<IrValue, X86Register> registerMap
  ) {
    var instructionToLiveRegistersMap = new HashMap<Instruction, Set<X86Register>>();
    if (!liveIntervals.isEmpty()) {
      var instructionList = liveIntervals.get(0)
                                         .instructionList();
      IntStream.range(
                   0,
  ...
  private static Set<X86Register> getLiveRegistersAtPoint(
      ProgramIr programIr,
      Collection<LiveInterval> liveIntervals,
      int index,
      Map<IrValue, X86Register> registerMap
  ) {
    return liveIntervals.stream()
                        ...
                        .filter(name -> !programIr.getGlobals()
                                                  .contains(name))
                        .map(registerMap::get)
                        .collect(Collectors.toUnmodifiableSet());
  }

  private static Set<IrValue> getLiveValuesAtPoint(
      Collection<LiveInterval> liveIntervals,
      int index
  ) {
    return liveIntervals.stream()
  ...
  private void computeMethodToLiveRegistersInfo(
      ProgramIr programIr,
      Map<Method, List<LiveInterval>> methodToLiveIntervalsMap
  ) {
