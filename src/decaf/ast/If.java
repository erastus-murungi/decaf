...
public class If extends Statement implements HasExpression {
    ...
    public String getSourceCode() {
        ...
        if ((elseBlock == null)) {
            return String.format("%s (%s) {\n    %s\n    }", DecafScanner.RESERVED_IF, test.getSourceCode(), indentedBlockString);
        } else {
            return String.format("%s (%s) {\n    %s\n    } %s {\n    %s        \n    }", DecafScanner.RESERVED_IF, test.getSourceCode(), indentedBlockString, DecafScanner.RESERVED_ELSE, Utils.indentBlock(elseBlock));
        }
    }
