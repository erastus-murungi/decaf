...
public class GenericSemanticChecker implements AstVisitor<Void> {
    ...
    private SymbolTable fields = new SymbolTable(null, SymbolTableType.Field, null);
    private SymbolTable methods = new SymbolTable(null, SymbolTableType.Method, null);
    ...
    public Void visit(FieldDeclaration fieldDeclaration, SymbolTable symbolTable) {
        ...
        for (Name name : fieldDeclaration.names) {
            if (symbolTable.isShadowingParameter(name.getLabel())) {
                ...
                getMethods().containsEntry(name.getLabel()) || getImports().contains(name.getLabel()) || getFields().containsEntry(name.getLabel()))) {
    ...
    public Void visit(MethodDefinition methodDefinition, SymbolTable symbolTable) {
        SymbolTable parameterSymbolTable = new SymbolTable(getFields(), SymbolTableType.Parameter, methodDefinition.block);
        SymbolTable localSymbolTable = new SymbolTable(parameterSymbolTable, SymbolTableType.Field, methodDefinition.block);
        if (getMethods().containsEntry(methodDefinition.methodName.getLabel()) || getImports().contains(methodDefinition.methodName.getLabel()) || getFields().containsEntry(methodDefinition.methodName.getLabel())) {
            ...
            getMethods().entries.put(methodDefinition.methodName.getLabel(), new MethodDescriptor(methodDefinition, parameterSymbolTable, localSymbolTable));
    ...
    public Void visit(MethodCall methodCall, SymbolTable symbolTable) {
        ...
        if (getMethods().getDescriptorFromValidScopes(methodName.getLabel()).isEmpty() && !getImports().contains(methodName.getLabel()))
