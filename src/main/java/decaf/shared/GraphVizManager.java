...
public class GraphVizManager {
  ...
  public static void printDominatorTree(Map<BasicBlock, BasicBlock> immediateDominators) {
    createDotGraph(
        writeDominatorGraph(immediateDominators),
        "dom"
    );
  }
  ...
  public static void printInterferenceGraph(
    ...
    createDotGraph(
        writeInterferenceGraph(interferenceGraph),
        fileName
    );
  }
  ...
  public static String writeInterferenceGraph(InterferenceGraph interferenceGraph) {
    ...
    for (var entry : interferenceGraph.getUniqueInterferenceGraphEdges()) {
      nodes.add(String.format(
          "   %s [shape=ellipse, style=filled, fillcolor=gray, label=\"%s\"];",
          entry.first()
               .hashCode(),
          escape(entry.first()
                      .toString()).replace(
    ...
    for (var entry : interferenceGraph.getUniqueMoveEdges()) {
      ...
      edges.add(String.format(
          "   %s -> %s [arrowhead=none, style=dotted, label=\"%s\"];",
          entry.first()
               .hashCode(),
          entry.second()
               .hashCode(),
  ...
  public static String writeCFG(
      String name,
      BasicBlock cfg,
      Function<BasicBlock, String> labelFunction
  ) {
    List<String> subGraphs = new ArrayList<>();
    subGraphs.add(String.format(
        "subgraph cluster_%s { \n label = %s",
        escape(name),
        escape(name)
    ));
    ...
    while (!stack.isEmpty()) {
      ...
      if (cfgBlock instanceof NOP) {
        nodes.add(String.format(
            "   %s [shape=record, style=filled, fillcolor=gray, label=%s];",
            cfgBlock.hashCode(),
        ...
        nodes.add(String.format(
            "   %s [shape=record, label=%s];",
            cfgBlock.hashCode(),
            "\"{<from_node>" + escape(labelFunction.apply(cfgBlock)) + "|{<from_true> T|<from_false>F}" + "}\""
        ));
  ...
  public static void printGraph(
      HashMap<String, BasicBlock> methodCFGBlocks,
      Function<BasicBlock, String> labelFunction
  ) {
    printGraph(
        methodCFGBlocks,
  ...
  public static void printGraph(HashMap<String, BasicBlock> methodCFGBlocks) {
    printGraph(
        methodCFGBlocks,
        (BasicBlock::getLinesOfCodeString),
        "cfg"
    );
  }
  ...
  public static void printGraph(
    ...
    methodCFGBlocks.forEach((k, v) -> graph[index[0]++] = writeCFG(
        k,
        v,
        labelFunction
    ));
...
//        }
  ...
  public static void printDominatorTree(
   ...
   */
  public int writeGraphToFile(
      byte[] img,
      String file
  ) {
    File to = new File(file);
    return writeGraphToFile(
        img,
        to
    );
  }
  ...
  private byte[] get_img_stream(
    ...
    try {
      img = File.createTempFile(
          "graph_",
          "." + type,
          new File(GraphVizManager.TEMP_DIR)
      );
