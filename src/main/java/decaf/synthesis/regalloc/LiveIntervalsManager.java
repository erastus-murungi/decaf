...
public class LiveIntervalsManager {
  ...
  public LiveIntervalsManager(
    ...
    methodToLiveIntervalsMap.put(
        method,
        liveIntervals
    );
  }

  private static int findFirstDefSlot(
  ...
  public static boolean liveIntervalsInterfere(
      LiveInterval a,
      LiveInterval b
  ) {
    var overlap = Math.abs(Math.min(
        a.startPoint(),
        b.endPoint() - 1
    ) - Math.max(
        a.startPoint(),
  ...
  public static <T> HashSet<T> difference(
      Set<T> first,
      Set<T> second
  ) {
  ...
  public List<Instruction> genInstructionsInLiveIntervalOf(
      IrValue irValue,
      Method method
  ) {
  ...
  public Map<IrValue, LiveInterval> getValueToLiveIntervalMappingForMethod(Method method) {
  ...
  private void computeMethodLivenessInformation(Method method) {
    ...
    basicBlocks.forEach(basicBlock -> computeLiveOutForSetBasicBlock(
        basicBlock,
        new LiveVariableAnalysis(method.getEntryBlock())
    ));
  }
  ...
  private void computeLiveOutForSetBasicBlock(
    ...
    for (var instruction : basicBlock.getInstructionListReversed()) {
      outLive = union(
          union(
              difference(
                  outLive,
                  defCache.get(instruction)
              ),
              refCache.get(instruction)
          ),
          outGlobal
      );
      instructionToLiveVariablesMap.put(
          instruction,
          outLive
      );
    }
  ...
  private List<LiveInterval> computeMethodLiveIntervals(Method method) {
    ...
    for (var variable : allVariables) {
      ...
      varToLiveInterval.put(
          variable,
          liveInterval
      );
    }
    methodToMappingOfValuesToLiveIntervals.put(
        method,
        varToLiveInterval
    );
    return new ArrayList<>(methodToMappingOfValuesToLiveIntervals.get(method)
                                                                 .values());
  }
  ...
  private void computeLiveIntervals() {
    for (var method : programIr.getMethods()) {
      methodToLiveIntervalsMap.put(
          method,
          computeMethodLiveIntervals(method)
      );
    }
  }

  private int findLastUseSlot(
      InstructionList instructionList,
      IrValue irValue
  ) {
    ...
    if (irValue instanceof IrMemoryAddress || irValue instanceof IrGlobalScalar || irValue instanceof IrGlobalArray) {
      ...
      for (; loc >= 0; loc--) {
        var instruction = instructionList.get(loc);
        if (instructionToLiveVariablesMap.getOrDefault(
                                             instruction,
    ...
    for (var instruction : instructionList) {
      for (var variable : allVariables) {
        ...
        if (isLiveAtPoint(
          ...
          System.out.print(Utils.coloredPrint(
              "\u2588",
