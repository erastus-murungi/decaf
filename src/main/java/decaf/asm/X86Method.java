...
public class X86Method extends ArrayList<X64Instruction> {
  ...
  public synchronized X86Method addLine(X64Instruction x64Instruction) {
    super.add(x64Instruction);
    return this;
  }

  public X86Method addAtIndex(
      int index,
      X64Instruction line
  ) {
    add(
  ...
  public void addAllAtIndex(
      int index,
      Collection<X64Instruction> lines
  ) {
    addAll(
        index,
        lines
  ...
  public String toString() {
    ...
    return buildStream().map(X64Instruction::toString)
