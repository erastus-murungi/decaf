...
public class BranchFoldingPass {
  ...
  private static boolean allAtIndexEqual(
      List<ArrayList<AST>> instructionLists,
  ...
  private static void mergeTails(List<BasicBlock> basicBlocks) {
    for (BasicBlock basicBlock : basicBlocks) {
      if (basicBlock.getPredecessors()
                    .size() > 1 && haveSameSuccessor(basicBlock.getPredecessors())) {
  ...
  private static void realignPointers(
  ...
  private static Optional<List<AST>> getCommonInstructions(List<BasicBlock> basicBlocks) {
    ...
    var minSize = basicBlocks.stream()
                             .map(basicBlock -> basicBlock.getAstNodes()
                                                          .size())
    ...
    if (minSize.isPresent()) {
      ...
      for (int indexOfInstruction = 0; indexOfInstruction < minSizeInt; indexOfInstruction++) {
        if (allAtIndexEqual(
          ...
          commonTailInstructions.add(collected.get(0)
                                              .get(indexOfInstruction));
        }
      }
    }
