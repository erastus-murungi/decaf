...
public class X64ValueResolver {
  ...
  public void setCurrentX64Method(@NotNull X64Method currentX64Method) {
  ...
  private X86ConstantValue resolveIrConstantValue(@NotNull IrConstant irConstant) {
    return checkNotNull(constants.computeIfAbsent(irConstant, k -> new X86ConstantValue(irConstant)));
  }

  boolean isUnResolvedArgumentIrValue(@NotNull IrValue irValue) {
    return irValue instanceof IrAssignableValue argumentIrRegister && getInitialArgumentLocations().containsKey(argumentIrRegister);
  }
  ...
  private X86Value localizeArgument(@NotNull IrAssignableValue argument) {
    ...
    currentX64Method.add(new X64BinaryInstruction(X64BinaryInstructionType.movq, initialArgumentLocation, localizedArgumentX86Value));
  ...
  private X64RegisterType genRegisterToSpill(
    ...
    for (var register : m.keySet()) {
      ...
      if (m.get(register).size() < fewestReferences) {
        fewestReferences = m.get(register).size();
        reg = register;
      }
    }
    checkState(reg != X64RegisterType.STACK);
    spillValuesToStack(reg, m.get(reg));
    updateIrValueMappingTo(irValue, reg);
    return reg;
  }
  ...
  private X64ArrayOperand resolveIrMemoryAddress(@NotNull IrMemoryAddress irMemoryAddress) {
    ...
    if (indexValue instanceof X86StackMappedValue) {
      ...
      indexRegister = (X86RegisterMappedValue) indexValue;
    }
    ...
    if (baseValue instanceof X86StackMappedValue) {
      ...
      baseRegister = new X86RegisterMappedValue(genRegisterToSpill(irMemoryAddress.getBaseAddress(), new ArrayList<>(List.of(indexRegister.getX64RegisterType()))), irMemoryAddress.getBaseAddress());
    } else {
  ...
  private boolean isStackMappedIrValue(@NotNull IrValue irValue) {
    return irValue instanceof IrAssignableValue && stackOffsets.get(currentMethod).containsKey(irValue);
  }
  ...
  public X86Value resolveIrValue(@NotNull IrValue irValue) {
    ...
    if (isStackMappedIrValue(irValue)) {
      return resolveStackMappedIrValue((IrAssignableValue) irValue);
    } else {
  ...
  private X86RegisterMappedValue resolveRegisterMappedIrValue(@NotNull IrAssignableValue irAssignableValue) {
    return registerMappedIrValues.get(currentMethod).get(irAssignableValue);
  }
    ...
    for (Instruction instruction : programIr.getPrologue()) {
      if (instruction instanceof GlobalAllocation globalAllocation) {
        if (globalAllocation.getValue() instanceof IrGlobalScalar irGlobalScalar) {
          globalScalars.put(irGlobalScalar, new X86GlobalValue(globalAllocation.getValue()));
        }
        ...
        constants.put(stringConstant, new X86ConstantValue(stringConstant));
      } else {
  ...
  private void mapParametersToLocations(
    ...
    var unusedRegisters = getUnusedRegisters(currentMethod, registerAllocator);
    ...
    for (int parameterIndex = 0; parameterIndex < method.getParameterNames().size(); parameterIndex++) {
      ...
      if (parameterIndex < unusedRegisters.size()) {
        destinations.put(parameter, new X86RegisterMappedValue(unusedRegisters.get(parameterIndex), parameter));
      } else {
  ...
  public X86Value resolveNextStackLocation(@NotNull X64RegisterType registerType) {
    if (temporarySaveLocations.containsKey(registerType))
      return new X86StackMappedValue(X64RegisterType.RBP, temporarySaveLocations.get(registerType));
    else {
      var newLocation = pushStack();
      temporarySaveLocations.put(registerType, newLocation);
      return new X86StackMappedValue(X64RegisterType.RBP, newLocation);
    }
  }
