...
public class X64AsmWriter implements AsmWriter {
    ...
    public X64AsmWriter(@NotNull ProgramIr programIr, @NotNull RegisterAllocator registerAllocator) {
        ...
        if (programIr.getMethods().get(0).entryBlock.getInstructionList().get(1).toString().contains("4.1"))
            System.out.println();
        emit();
    ...
    public void emitInstruction(@NotNull MethodEnd methodEnd) {
        var space = new X86ConstantValue(new IrIntegerConstant((long) roundUp16(-x64ValueResolver.getStackOffset()), Type.Int));
    ...
    public void emitInstruction(@NotNull ReturnInstruction returnInstruction) {
        if (returnInstruction.getReturnAddress()
                             .isPresent())
            x64Method.addLine(new X64BinaryInstruction(X64BinaryInstructionType.movq,
                    resolveIrValue(returnInstruction.getReturnAddress().get()),
                    X86RegisterMappedValue.unassigned(X64RegisterType.RAX)));
    }
