...
import edu.mit.compilers.cfg.ShortCircuitProcessor;
...
public class Utils {
  ...
  public static void printSsaCfg(
    ...
    GraphVizPrinter.printGraph(copy, (basicBlock -> basicBlock.getInstructionList().stream().map(Instruction::toString)
                                                              .collect(Collectors.joining("\n"))), filename);
  }

  public static Set<IrAssignableValue> getAllLValuesInInstructionList(InstructionList instructionList) {
    return instructionList.stream().flatMap(instruction -> instruction.getAllLValues().stream())
  ...
  public static Set<IrAssignableValue> getLAllRegisterAllocatableValuesInInstructionList(Collection<Instruction> instructionList) {
    return instructionList.stream().flatMap(instruction -> instruction.getAllRegisterAllocatableValues().stream())
                          .collect(Collectors.toSet());

  }

  public static Set<IrAssignableValue> getAllLValuesInBasicBlocks(List<BasicBlock> basicBlocks) {
    return (basicBlocks.stream().flatMap(basicBlock -> basicBlock.getInstructionList().stream())
                       .flatMap(instruction -> instruction.getAllLValues().stream())
                       .collect(Collectors.toUnmodifiableSet()));
  }

  public static Set<IrRegister> getAllVirtualRegistersInBasicBlocks(List<BasicBlock> basicBlocks) {
    return (basicBlocks.stream().flatMap(basicBlock -> basicBlock.getInstructionList().stream())
                       .flatMap(instruction -> instruction.getAllVirtualRegisters().stream())
                       .collect(Collectors.toUnmodifiableSet()));
  }
  ...
  private String removeParentheses(String s) {
    s = s.strip();
    if (s.startsWith("(")) {
      return removeParentheses(s.strip().substring(1, s.length() - 1));
    }
    return s;
  }
