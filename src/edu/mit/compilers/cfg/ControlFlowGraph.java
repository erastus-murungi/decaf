...
public class ControlFlowGraph {
    ...
    public ControlFlowGraph(Program program, GlobalDescriptor globalDescriptor) {
    ...
    private void catchFalloutError(MethodDescriptor methodDescriptor) {
        ...
        if (returningPaths.size() != nonReturningPaths.size()) {
            errors.addAll(nonReturningPaths
                    .stream()
                    .map(basicBlock -> new DecafException(methodDefinition.tokenPosition,
    ...
    public void build() {
        ...
        visitProgram(program, globalDescriptor.globalVariablesSymbolTable);


        methodNameToEntryBlock.forEach((k, v) -> removeNOPs(v.getSuccessor(), methodNameToExitNop.get(k)));

        methodNameToEntryBlock.forEach((k, v) -> {
            maximalVisitor.exitNOP = methodNameToExitNop.get(k);
            checkNotNull(v.getSuccessor());
            maximalVisitor.visit(v.getSuccessor());
            catchFalloutError((MethodDescriptor) globalDescriptor.methodsSymbolTable
                    .getDescriptorFromValidScopes(k)
                    .orElseThrow());
        });
        removeNOPs(prologue, (NOP) prologue.getSuccessor());
        ...
        maximalVisitor.exitNOP = (NOP) prologue.getSuccessor();
    ...
    private BasicBlocksPair dispatch(AST ast, SymbolTable symbolTable) {
        if (ast instanceof MethodDefinition methodDefinition) {
            return visitMethodDefinition(methodDefinition, symbolTable);
            ...
            return visitFor(forStatement, symbolTable);
        } else if (ast instanceof While whileLoop) {
            return visitWhile(whileLoop, symbolTable);
            ...
            throw new IllegalStateException(ast.getClass().getSimpleName());
        }
    }
    ...
    public BasicBlocksPair visitMethodDefinition(MethodDefinition methodDefinition, SymbolTable symbolTable) {
        ...
        for (var param : methodDefinition.parameterList) {
            var placeholder = dispatch(param, symbolTable);
        ...
        var methodBody = dispatch(methodDefinition.block, symbolTable);
    ...
    public BasicBlocksPair visitFor(For forStatement, SymbolTable symbolTable) {
    ...
    public BasicBlocksPair visitWhile(While whileStatement, SymbolTable symbolTable) {
        loopToBreak.push(new ArrayList<>());
        ...
        // If true, run the block.
        var truePair = dispatch(whileStatement.body, symbolTable);
    ...
    public void visitProgram(Program program, SymbolTable symbolTable) {
        var curPair = new BasicBlocksPair(prologue, new NOP("global NOP", NOP.NOPType.NORMAL));
        prologue.setSuccessor(curPair.endBlock);
        for (var import_ : program.importDeclarationList) {
            BasicBlocksPair placeholder = dispatch(import_, symbolTable);
        ...
        for (var field : program.fieldDeclarationList) {
            BasicBlocksPair placeholder = dispatch(field, symbolTable);
            curPair.endBlock.setSuccessor(placeholder.startBlock);
            placeholder.startBlock.addPredecessor(curPair.endBlock);
            curPair = placeholder;
        }
        for (var method : program.methodDefinitionList) {
            ...
            methodNameToEntryBlock.put(method.methodName.getLabel(), dispatch(method, symbolTable).startBlock);
            methodNameToExitNop.put(method.methodName.getLabel(), exitNop);
        }
    }

    public BasicBlocksPair visitBlock(Block block, SymbolTable symbolTable) {
        NOP initial = new NOP();
        NOP exit = new NOP();
        ...
        for (Statement statement : block.statementList) {
            ...
            if (statement instanceof Return returnStatement) {
                BasicBlocksPair returnPair = dispatch(returnStatement, symbolTable);
            ...
            // recurse normally for other cases
                ...
                BasicBlocksPair placeholder = dispatch(statement, symbolTable);
                curPair.endBlock.setSuccessor(placeholder.startBlock);
                placeholder.startBlock.addPredecessor(curPair.endBlock);
                curPair = placeholder;
            }
    ...
    public BasicBlocksPair visitIf(If ifStatement, SymbolTable symbolTable) {
        // always end with nop
        final NOP exit = new NOP();

        // If true, run the block.
        BasicBlocksPair truePair = dispatch(ifStatement.ifBlock, symbolTable);
        ...
        if (ifStatement.elseBlock != null) {
            BasicBlocksPair falsePair = dispatch(ifStatement.elseBlock, symbolTable);
